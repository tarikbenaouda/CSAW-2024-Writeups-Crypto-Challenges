from oracle import encrypt, is_padding_ok, BLOCK_SIZE, _decrypt, test_padding_oracle
from pwn import *
...
enhance = ( 
            [ 32 ]
	        + [ x for x in range( 97,124 )]
            + [ 44 , 46]
            + [ x for x in range( 65, 92 )]
            + [ x for x in range( 48, 58 )]
            + [ x for x in range(48 )]
            + [ x for x in range( 58, 65 )]
            + [ x for x in range( 92, 97 )]
            + [ x for x in range( 124, 256 )])
print(len(enhance))
                  

def attack(io, ciphertext ):
...
def test_the_attack(io):
    cracked_ct = attack(io,unhexlify(out))
    print(cracked_ct)
    assert( cracked_ct == b'YELLOW SUBMARINE' )
